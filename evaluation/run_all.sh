...
saved_results_dir=${dir}/ttrl_saved
save_path_dir=${saved_results_dir}/evaluation
solver_path_dir=${saved_results_dir}/Solver_ttrl
base_model_name=Qwen3-4B-Base
base_model_eval_results_dir=${save_path_dir}/${base_model_name}
eval_step=15
temperature=0.6
num_iter=5
prefix=ttrl_hybrid_data_qr_R_Zero_gqttrl_icl_Qwen3-4B-Base
...
exec > >(tee -a "${WORKING_DIR}/eval_logs/eval_${prefix}-$(now).log") 2>&1
TASKS=(
  "temp_data"
  "greedy_data"
)
...
start_additional_eval_job() {
    ...
    CUDA_VISIBLE_DEVICES="${gpu_id}" python "${eval_script}" --model_path "${model_path}" --model_name "${model_name}" --save_path_dir "${eval_saved_path_dir}" --data_path_dir "${data_dir}" &
    local pid=$!
...
start_math_task_job() {
    ...
    # Start evaluation script on GPU
    CUDA_VISIBLE_DEVICES="${gpu_id}" python eval_all_math_para.py --model_path "${model_path}" --model_name "${model_name}" --dataset "${dataset}" --save_path_dir "${eval_saved_path_dir}" --n_samples "${n_samples}" --temperature "${temp}" --data_path_dir "${data_dir}" &
    local pid=$!
...
python tb.py --prefix=$prefix --step=$eval_step --temperature=$temperature --eval_results_dir="${eval_saved_path_dir}" --tb_path_dir="${tb_path_dir}"
