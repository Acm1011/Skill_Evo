...
dir="${SE_BASE_DIR:-/home/ycy/data3}"
...
if [ ! -d "${base_model_path}" ]; then
    ...
    if [ -f "${base_model_eval_results_dir}/temp_data_Overall_results.jsonl" ] && \
       [ -f "${base_model_eval_results_dir}/greedy_data_Overall_results.jsonl" ]; then
        echo "基础模型 ${base_model_name} 评测结果已存在: ${base_model_eval_results_dir}，跳过评测。"
    else
        echo "基础模型 ${base_model_name} 评测结果不完整，需要重新评测"
...
start_additional_eval_job() {
    ...
    CUDA_VISIBLE_DEVICES="${gpu_id}" python "${eval_script}" --model_path "${model_path}" --model_name "${model_name}" --save_path_dir "${target_save_dir}" --data_path_dir "${data_dir}" &
    local pid=$!
