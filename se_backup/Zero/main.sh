...
echo "开始第一轮训练..."
echo "训练 Challenger..."
bash challenger.sh $exp_name $base_model_path $base_model_path $challenger_training_steps || {
    echo "Error: 第一轮 Challenger 训练失败"
    exit 1
}

echo "训练 Solver..."
bash solver.sh $exp_name ${challenger_path_dir}/${exp_name}/ckpts//global_step_${challenger_training_steps}/actor/huggingface $base_model_path $solver_training_steps || {
