...
PREPARE_SCRIPT="${WORKING_DIR}/prepare_test_data.py"
RETRIEVER_START_SCRIPT="${SCRIPT_DIR}/start_retriever_server.sh"

usage() {
    cat <<'USAGE'
Usage:
  bash skill_src/Zero/prepare_test_data_all_versions.sh <memory_dir> [input_jsonl ...]

Arguments:
  memory_dir     Directory containing memory_after_sol_v*.jsonl
  input_jsonl    Optional input jsonl files. Defaults to:
                 /home/ycy/sdi/data/temp_data.jsonl
                 /home/ycy/sdi/data/greedy_data.jsonl

Environment:
  SE_PREPARE_TEST_TOP_K       Retriever top-k, default 3
  SE_PREPARE_TEST_OUTPUT_DIR  Output dir, default parent of <memory_dir>
...
if [ "$#" -gt 0 ]; then
    ...
    INPUTS=(
        "${SE_PREPARE_TEST_INPUT_1:-/home/ycy/sdi/data/temp_data.jsonl}"
        "${SE_PREPARE_TEST_INPUT_2:-/home/ycy/sdi/data/greedy_data.jsonl}"
    )
fi
...
OUTPUT_DIR="${SE_PREPARE_TEST_OUTPUT_DIR:-$(dirname "${MEMORY_DIR}")}"
TOP_K="${SE_PREPARE_TEST_TOP_K:-3}"
...
if curl -sf "http://${RETRIEVER_HOST}:${RETRIEVER_PORT}/health" >/dev/null 2>&1; then
    echo "[prepare_test_data_all_versions] retriever already healthy: http://${RETRIEVER_HOST}:${RETRIEVER_PORT}/health"
else
    echo "[prepare_test_data_all_versions] starting retriever server"
...
for memory_jsonl in "${MEMORY_FILES[@]}"; do
    ...
    echo "[prepare_test_data_all_versions] processing ${memory_name}"
    for input_path in "${INPUTS[@]}"; do
        stem="$(basename "${input_path}" .jsonl)"
        base_jsonl="${OUTPUT_DIR}/${stem}_skill.jsonl"
        base_parquet="${OUTPUT_DIR}/${stem}_skill.parquet"
        versioned_jsonl="${OUTPUT_DIR}/${stem}_skill_v${version}.jsonl"
        versioned_parquet="${OUTPUT_DIR}/${stem}_skill_v${version}.parquet"
        ...
        if [ ! -f "${base_jsonl}" ] || [ ! -f "${base_parquet}" ]; then
            echo "Error: expected output files missing for ${input_path} with ${memory_name}" >&2
            exit 1
        fi

        mv -f "${base_jsonl}" "${versioned_jsonl}"
        mv -f "${base_parquet}" "${versioned_parquet}"
        echo "  ${input_path} -> ${versioned_jsonl}"
        echo "  ${input_path} -> ${versioned_parquet}"
    done
done

echo "[prepare_test_data_all_versions] completed. output_dir=${OUTPUT_DIR}"
