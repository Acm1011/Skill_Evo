#!/usr/bin/env bash
# =============================================================================
# Synthesizer_reward_only.sh - Synthesizer reward-only 执行器（不训练参数）
# =============================================================================
...
#   Step 3. 仅后半卡再启 reward rollout + Step 4 reward-only：后半卡 vLLM 与 main_synthesizer 同时起，随后再等待各
#   /health（与 Ray/模型加载重叠，省时间；须 trainer.val_before_train=False 以免首步早于 HTTP reward）
#   退出时自动清理 Step 3 的 rollout server
#
#   SE_SYNTH_SKIP_OFFLINE=1|true|yes（由 main_o_no_synth.sh 在 SE_RESUME 且已有 merged/train_data 时仅对本次子进程设置）:
...
# reward-only：完全不启动 main_synthesizer，仅生成 skill 并计算 reward_info 写盘
echo "[reward-only] 模式：不启动 main_synthesizer，仅调用 synth_reward_only_driver.py"
...
# ===========================================================================
# Step 3: 仅后半卡再启 rollout（供 reward）
# ===========================================================================
echo ""
...
echo ""
echo "  后半卡: 起 reward rollout vLLM；前半卡: 供 reward-only 生成 skill"
echo "  Synthesizer GPUs: ${SYNTH_GPU_IDS} (${N_SYNTH_GPUS} 张)  temp=${query_temperature}"
...
echo "========== Step 4: reward-only driver（无 main_synthesizer）=========="
CUDA_VISIBLE_DEVICES=${SYNTH_GPU_IDS} python3 -m ${SE_CODE_MODULE}.synth_reward_only_driver \
    --train-file "${TRAIN_DATA_FILE}" \
    --synth-model-path "${synthesizer_model_path}" \
    --storage-path "${storage_path}" \
    --step "${synthesizer_training_steps}" \
    --use-skill-type "${use_skill_type}" \
    --random-q-coef "${random_q_coef}" \
    --max-prompt-length "${max_prompt_length}" \
    --max-new-tokens "${max_response_length}" \
    --temperature "${query_temperature}" \
    --top-p "${query_top_p}" \
    --top-k "${query_top_k}" || {
    echo "Error: reward-only driver 执行失败" >&2
    exit 1
}

echo ""
echo "=============================================="
echo "  ${exp_name} Synthesizer reward-only 完成"
echo "  reward path: ${storage_path}/reward_info/exp_data_step_*.jsonl"
echo "=============================================="
