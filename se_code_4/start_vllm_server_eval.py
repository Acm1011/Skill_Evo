...
from se_code.utils import process_single_R_Zero, process_single_Rule, process_single_q_eval
...
def hello():
    ...
    if question_reward == "q_eval":
        with open('q_eval_system.txt', 'r') as f:
            q_eval_sys = f.read()
        with open('q_eval_user.txt', 'r') as f:
            q_eval_user = f.read()
    for i,q in enumerate(questions):
        if q:
            if question_reward == "q_eval":
                ...
                valid_chats.append([
                    {'role':'system', 'content':q_eval_sys},
                    {'role': 'user',   'content': q_eval_user.format(QUESTION=q)}
                ])
            else:
                valid_indices.append(i)
                valid_questions.append(q)
    ...
    def process_single(idx, question, response):
        if question_reward == 'R_Zero':
            return process_single_R_Zero(idx, question, response)
        elif question_reward =='Rule':
            return process_single_Rule(idx, question, response)
        elif question_reward == "q_eval":
            return process_single_q_eval(idx, question, response)
        else:
