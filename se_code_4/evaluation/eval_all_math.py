...
def compute_score(response: str, gts: list):
    ...
    return rule_correct, check_correct, is_check, str(pred),check_rsp
...
def main(args):
    ...
    if args.model.endswith("huggingface"):
        model_name = args.model.split("/")[-5]

    else:
        model_name = args.model.split("/")[-1]
    
    print(f'model[{model_name}] with dataset[{args.dataset}] set sampling parameters: max_tokens={args.max_gen_len}, temperature={args.temperature}, top_p={args.top_p}, top_k={args.top_k} , n_samples={n_samples}')
    ...
    for data_source, data in dataset.groupby('data_source'):
        ...
        for i, (_, data_item) in enumerate(data.iterrows()):
            ...
            for idx, rsp in enumerate(responses):
                rule_correct, check_correct, is_check, pred,check_rsp = compute_score(rsp, gts)
        ...
        print(f'model[{model_name}] with dataset[{args.dataset}] evaluate {data_source} dataset results saved in {args.save_path}/{data_source}_eval_results.jsonl successfully')
    ...
    print(f'model[{model_name}] with dataset[{args.dataset}] Overall results saved in {args.save_path}/{args.dataset}_Overall_results.jsonl successfully')
