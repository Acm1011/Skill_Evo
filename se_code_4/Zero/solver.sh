...
#gen_query_num=8
echo "开始生成查询数据..."
bash ${SCRIPT_DIR}/gen_query.sh $exp_name $gen_query_num $challenger_model_path ${challenger_path_dir} ${solver_path_dir} $gen_question_func || {
...
cd ${WORKING_DIR}
# 启动训练进程并记录PID
CUDA_VISIBLE_DEVICES=4,5,6,7 python3 -m se_code.main_solver_dapo \
