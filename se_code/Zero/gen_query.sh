...
# 启动并行查询生成进程
CUDA_VISIBLE_DEVICES=0 python -m se_code.challenger_generate_query --model "$challenger_model_path" --suffix 0 --num_samples "$num_samples" --storage_path="$storage_path" --get_prompts_func="$gen_question_func" --prompt_path="$prompt_path" &
GEN_PID1=$!

CUDA_VISIBLE_DEVICES=1 python -m se_code.challenger_generate_query --model "$challenger_model_path" --suffix 1 --num_samples "$num_samples" --storage_path="$storage_path" --get_prompts_func="$gen_question_func" --prompt_path="$prompt_path" &
GEN_PID2=$!

CUDA_VISIBLE_DEVICES=2 python -m se_code.challenger_generate_query --model "$challenger_model_path" --suffix 2 --num_samples "$num_samples" --storage_path="$storage_path" --get_prompts_func="$gen_question_func" --prompt_path="$prompt_path" &
GEN_PID3=$!

CUDA_VISIBLE_DEVICES=3 python -m se_code.challenger_generate_query --model "$challenger_model_path" --suffix 3 --num_samples "$num_samples" --storage_path="$storage_path" --get_prompts_func="$gen_question_func" --prompt_path="$prompt_path" &
GEN_PID4=$!
