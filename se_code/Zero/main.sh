...
echo "所有训练完成！"
echo "开始评估..."
bash "${WORKING_DIR}/evaluation/eval_all_run.sh" "${variant}" "${solver_eval_temperature}" "${solver_eval_step}" "${solver_eval_num_iter}" "${base_model_name}" || {
    echo "Error: 评估失败"
    exit 1
}
echo "评估完成！"