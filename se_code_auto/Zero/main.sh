...
base_model_name="${SE_BASE_MODEL_NAME:-Qwen2.5-Math-7B}"
...
bash "$eval_script" "${variant}" "${solver_eval_temperature}" "${solver_eval_step}" "${solver_eval_num_iter}" "${base_model_name}" || {
    echo "Error: 评估失败"
    exit 1
}
eval_script="${SE_EVAL_SCRIPT:-${WORKING_DIR}/evaluation/eval_all_run.sh}"
