...
# export CC=gcc-12
# export CXX=g++-12
# export TRITON_CC=gcc-12
# export TRITON_CXX=g++-12
#echo "已设置Triton使用GCC-12进行编译，保持JIT功能"
...
variant=entropy_prompt2_se-Zero_${base_model_name}
exp_name=${variant}-V1
challenger_training_steps=5
solver_training_steps=20
solver_retrain_steps=15
tmp=${saved_results_dir}/${variant}/tmp
mkdir -p ${saved_results_dir} ${challenger_path_dir} ${solver_path_dir} ${tensorboard_dir} ${tmp}
topics_path=$tmp/topics.json
cp -r ${WORKING_DIR}/se_code/prompt2/all_topic_annotations.json $topics_path
cd ${WORKING_DIR}
echo "开始第一轮训练..."
echo "训练 Challenger..."
bash ${SCRIPT_DIR}/challenger_entropy.sh $exp_name $base_model_path $base_model_path $challenger_training_steps $topics_path || {
    echo "Error: 第一轮 Challenger 训练失败"
    exit 1
}

echo "训练 Solver..."
bash ${SCRIPT_DIR}/solver.sh $exp_name ${challenger_path_dir}/${exp_name}/ckpts//global_step_${challenger_training_steps}/actor/huggingface $base_model_path $solver_training_steps $topics_path || {
...
for iter in {2..6}; do
    ...
    bash ${SCRIPT_DIR}/challenger_entropy.sh \
        ${exp_name} ${prev_challenger_model_path} \
        ${prev_solver_model_path} $challenger_training_steps $topics_path || {
    ...
    bash ${SCRIPT_DIR}/solver.sh \
        ...
        ${prev_solver_model_path} $solver_training_steps $topics_path || {
