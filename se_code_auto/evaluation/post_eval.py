...
def process_example(preds, gt):
    try:
        ...
        gpt_response.raise_for_status()  # 如果HTTP状态码不是2xx，抛出异常触发重试
        ...
        # 重新抛出异常，让 @retry 装饰器捕获并触发重试
        # 只有在所有重试都失败后，异常才会最终传播出去
        raise e
