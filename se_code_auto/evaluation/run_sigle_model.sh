#!/bin/bash
# sigle model multi step evaluation script
export VLLM_DISABLE_COMPILE_CACHE=1
save_path_dir=/root/users/ycy/saved_results/evaluation
...
model_name=filter_data_Qwen3-4B-Base
step_list=(
    5
    10
    15
...
start_model_job() {
    ...
    local model_path="$3"
    local step="$4"
    echo "==> [$(date '+%Y-%m-%d %H:%M:%S')] Starting model [${model_name}] with step [${step}] on GPUs [${gpu_id_0}] and [${gpu_id_1}]"
    
    # Start temp_data task on first GPU
    CUDA_VISIBLE_DEVICES="${gpu_id_0}" python eval_all_math_step.py --model_path "${model_path}" --model_name "${model_name}" --dataset "${TASKS[0]}" --save_path_dir "${save_path_dir}" --step "${step}" --n_samples 32 --temperature 1.0  &
    local pid_0=$!
    
    # Start greedy_data task on second GPU
    CUDA_VISIBLE_DEVICES="${gpu_id_1}" python eval_all_math_step.py --model_path "${model_path}" --model_name "${model_name}" --dataset "${TASKS[1]}" --save_path_dir "${save_path_dir}" --step "${step}" --n_samples 1 --temperature 0.0  &
    local pid_1=$!
    ...
    if kill -0 "$pid_0" 2>/dev/null && kill -0 "$pid_1" 2>/dev/null; then
        ...
        pids["${gpu_id_0}"]="$pid_0"
        pids["${gpu_id_1}"]="$pid_1"
        model_gpu_mapping["${gpu_id_0}"]="${step}"
        model_gpu_mapping["${gpu_id_1}"]="${step}"
...
check_completed_jobs() {
    ...
    for gpu_id in "${!pids[@]}"; do
        ...
        if ! kill -0 "$pid" 2>/dev/null; then
            local step="${model_gpu_mapping[$gpu_id]}"
            
            # Check if we've already processed this GPU
            if [ -z "${task_completed[$gpu_id]}" ]; then
                echo "==> [$(date '+%Y-%m-%d %H:%M:%S')] GPU [${gpu_id}] job finished for step [${step}] with PID [${pid}]"
                ...
                echo "==> [$(date '+%Y-%m-%d %H:%M:%S')] Running post_eval_step for model_name [${model_name}] and step [${step}] on dataset [${dataset}] with n_samples [${n_samples}]..."
                python post_eval_step.py --save_path_dir "${save_path_dir}" --dataset "${dataset}" --model_name "${model_name}" --n_samples "${n_samples}" --step "${step}"
                echo "==> [$(date '+%Y-%m-%d %H:%M:%S')] Post-processing_step completed for GPU [${gpu_id}] (model [${model_name}] with step [${step}], dataset [${dataset}])"
...
free_gpu() {
    ...
    echo "==> [$(date '+%Y-%m-%d %H:%M:%S')] Freed GPU [${gpu_id}] (was running model [${model_name}] with step [${step}])"
}
...
for step in "${step_list[@]}"; do
    echo "==> [$(date '+%Y-%m-%d %H:%M:%S')] Processing model: ${model_name} with step: ${step}"
    model_path=${solver_path_dir}/${model_name}/ckpts/global_step_${step}/actor/huggingface
    ...
    echo "==> [$(date '+%Y-%m-%d %H:%M:%S')] DEBUG: Using model_path=$model_path, model_name=$model_name, step=$step"
    ...
    while true; do
        ...
        if [ ${#available_gpus[@]} -ge 2 ]; then
            ...
            if [[ "${available_gpus[0]}" =~ ^[0-9]+$ ]] && [[ "${available_gpus[1]}" =~ ^[0-9]+$ ]]; then
                ...
                if start_model_job "${available_gpus[0]}" "${available_gpus[1]}" "$model_path" "$step"; then
                    echo "==> [$(date '+%Y-%m-%d %H:%M:%S')] Successfully started model [${model_name}] with step [${step}]"
                    break
                else
                    echo "==> [$(date '+%Y-%m-%d %H:%M:%S')] Failed to start model [${model_name}] with step [${step}], retrying..."
                    retry_count=$((retry_count + 1))
                    if [ $retry_count -ge $max_retries ]; then
                        echo "==> [$(date '+%Y-%m-%d %H:%M:%S')] Max retries reached for model [${model_name}] with step [${step}], skipping..."
                        break
                    fi
                    sleep 5
                fi
    ...
    echo "==> [$(date '+%Y-%m-%d %H:%M:%S')] Started model: ${model_name} with step [${step}]"
done
...
python tb_step.py --model_name ${model_name} 
