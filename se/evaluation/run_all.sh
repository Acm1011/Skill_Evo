...
model_list=(
  "Qwen3-4B-Base"
  "RR-Zero_Qwen3-4B-Base-V1"
  "RR-Zero_Qwen3-4B-Base-V2"
  "RR-Zero_Qwen3-4B-Base-V3"
  "RR-Zero_Qwen3-4B-Base-V4"
  "RR-Zero_Qwen3-4B-Base-V5"
)
eval_step=15
TASKS=(
  "temp_data"
...
start_model_job() {
    ...
    CUDA_VISIBLE_DEVICES="${gpu_id_0}" python eval_all_math_para.py --model_path "${model_path}" --model_name "${model_name}" --dataset "${TASKS[0]}" --save_path_dir "${save_path_dir}" --n_samples 32 --temperature 1.0  &
    local pid_0=$!
    ...
    for gpu_id in "${!pids[@]}"; do
        ...
        if ! kill -0 "$pid" 2>/dev/null; then
            ...
            if [ -z "${task_completed[$gpu_id]}" ]; then
                ...
                echo "==> [$(date '+%Y-%m-%d %H:%M:%S')] Running post_eval for [${model_name}] on dataset [${dataset}] with n_samples [${n_samples}]..."
                python post_eval.py --save_path_dir "${save_path_dir}" --dataset "${dataset}" --model_name "${model_name}" --n_samples "${n_samples}"
                echo "==> [$(date '+%Y-%m-%d %H:%M:%S')] Post-processing completed for GPU [${gpu_id}] (model [${model_name}], dataset [${dataset}])"
