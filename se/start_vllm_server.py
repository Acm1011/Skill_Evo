...
def hello():
    ...
    def process_single(idx,question, response):
        ...
        for out in response.outputs:
            ...
            result = extract_boxed_content(out.text)
            all_labels.append(result if result is not None else 'None')
        ...
        return {
            ...
            'labels2cnt': answer_counts,
            'all_labels': all_labels,
            'valid_labels': list(answer_counts.keys()),
            'boxed_labels': results,
        ...
        try:
            if q:
                ...
                if response_idx >= len(responses):
                    print(f'[server] WARNING: response_idx ({response_idx}) >= len(responses) ({len(responses)}) for question {idx}')
                    results_all.append({
                        'idx': idx,
                        'question': q,
                        'answer':' ',
                        ...
                        'valid_labels_cnt': 0,
                        'all_labels_cnt': 0,
                        'error': f'response index out of range: {response_idx} >= {len(responses)}'
                    })
                    continue
