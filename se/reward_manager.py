...
def generate_results(data, storage_path: str):
    # 将数据分成4份
    datas = split_list(data, 4)
    random_names = [generate_temp_filename(storage_path=storage_path, prefix=f"temp_{i}") for i in range(4)]
...
class ChallengerRewardManager(AbstractRewardManager):
    ...
    def compute_score(self, predicts, storage_path:str, topics, step=0):
        ...
        for i in range(len(final_results)):
            ...
            base_score = final_results[i].get("uncertainty_reward", 0)
            ...
            saved_results.append(
                {
                    ...
                    "pseudo_label(majority)":final_results[i]['answer'],
                    ...
                    'format_reward': 1 if final_results[i]['question'] else 0,
                    "final_reward": final_score,
                    "all_labels":final_results[i]['all_labels'],
                    'labels2cnt':final_results[i]['labels2cnt'],
                    'boxed_labels':final_results[i]['boxed_labels'],
            ...
            reward_infos.append(
                {
                    ...
                    "topic": topics[i] if self.num_examine == 0 else data[i].non_tensor_batch['data_source'],
                    "question": prompts[i],
