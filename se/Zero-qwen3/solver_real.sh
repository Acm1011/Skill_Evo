...
split_idx="$4"
data_iter=$(((split_idx - 1) % 8 + 1))
echo "[exp_name]:${exp_name}"
echo "[solver_model_path]: ${solver_model_path}"
echo "[solver_training_steps]: ${solver_training_steps}"    
echo "[split_idx]: ${split_idx}"
echo "[data_iter]: ${data_iter}"
# Paths
data_path_dir=/root/users/ycy/data/DeepScaleR-40K-split
TRAIN_FILE=${data_path_dir}/DeepScaleR-40K-split-V${data_iter}.parquet
...
if [ -z "$split_idx" ]; then
    echo "Error: split_idx 不能为空"
    exit 1
fi

if ! [[ "$split_idx" =~ ^[0-9]+$ ]]; then
    echo "Error: split_idx 必须是数字，当前值: $split_idx"
    exit 1
fi
...
storage_path=${real_solver_path_dir}/${exp_name}
CKPTS_DIR=${storage_path}/ckpts/
tensorboard_path=${tensorboard_dir}/Real-Solver-${exp_name}
