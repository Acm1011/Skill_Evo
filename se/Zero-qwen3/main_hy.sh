...
real_solver_path_dir=${saved_results_dir}/Real-Solver
...
variant=Hy-R-Zero_${base_model_name}
exp_name=${variant}-V1
challenger_training_steps=5
solver_training_steps=20
solver_retrain_steps=15

mkdir -p ${saved_results_dir} ${challenger_path_dir} ${solver_path_dir} ${tensorboard_dir} ${real_solver_path_dir}
# echo "开始第一轮训练..."
# echo "训练 Challenger..."
...
echo "real_data训练..."
bash ${SCRIPT_DIR}/solver_real.sh $exp_name ${solver_path_dir}/${exp_name}/ckpts/global_step_${solver_retrain_steps}/actor/huggingface $solver_training_steps 1 || {
    echo "Error: real_data训练失败"
    exit 1
}
...
for iter in {2..8}; do
    ...
    prev_real_solver_model_path=${real_solver_path_dir}/${prev_exp_name}/ckpts/global_step_${solver_training_steps}/actor/huggingface
    cur_real_solver_model_path=${real_solver_path_dir}/${exp_name}/ckpts/global_step_${solver_training_steps}/actor/huggingface
    ...
    bash ${SCRIPT_DIR}/challenger.sh \
        ...
        ${prev_real_solver_model_path} $challenger_training_steps || {
    ...
    bash ${SCRIPT_DIR}/solver.sh \
        ...
        ${prev_real_solver_model_path} $solver_training_steps || {
        echo "Error: 第 ${iter} 轮 Solver 训练失败"
        exit 1
    }

    echo "real_data训练..."
    bash ${SCRIPT_DIR}/solver_real.sh $exp_name ${cur_solver_model_path} $solver_training_steps $iter || {
        echo "Error: 第 ${iter} 轮 real_data 训练失败"
        exit 1
    }
    echo "第 ${iter} 轮训练完成"
done
