...
usage() {
    cat <<USAGE
Usage:
  bash baselines/ReasoningBankMath/scripts/run_gen_traj.sh [options]
...
OUTPUT="${REPO_ROOT}/baselines/ReasoningBankMath/outputs/trajectories.jsonl"
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        -h|--help) usage; exit 0 ;;
        *) echo "[rbm-gen-traj] unknown arg: $1" >&2; usage; exit 2 ;;
    esac
done

if [[ -z "${DATA_PATH}" || -z "${END_IDX}" ]]; then
    echo "[rbm-gen-traj] --data-path and --end are required" >&2
    exit 2
fi
...
for ((i = 0; i < SE_ROLLOUT_N_SERVERS; i++)); do
    ...
    if [[ "${ok}" -ne 1 ]]; then
        echo "[rbm-gen-traj] rollout health check failed: ${url}" >&2
        exit 1
    fi
done

CMD=(
  python -m baselines.ReasoningBankMath gen-traj
  --data-path "${DATA_PATH}"
  --start "${START_IDX}"
  --end "${END_IDX}"
  --output "${OUTPUT}"
