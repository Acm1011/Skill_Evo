...
usage() {
    cat <<USAGE
Usage:
  bash baselines/ReasoningBankMath/scripts/run_build_memory_with_rollout.sh [options]
...
OUT_PATH="${REPO_ROOT}/baselines/ReasoningBankMath/outputs/memory_bank.jsonl"
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        -h|--help) usage; exit 0 ;;
        *) echo "[rbm-build-memory] unknown arg: $1" >&2; usage; exit 2 ;;
    esac
done

if [[ -z "${TRAJ_PATH}" ]]; then
    echo "[rbm-build-memory] --trajectories is required" >&2
    exit 2
fi

mkdir -p "$(dirname "${OUT_PATH}")" "${ROLLOUT_LOG_DIR}"
...
for ((i = 0; i < SE_ROLLOUT_N_SERVERS; i++)); do
    ...
    if [[ "${ok}" -ne 1 ]]; then
        echo "[rbm-build-memory] rollout health check failed: ${url}" >&2
        exit 1
    fi
done
