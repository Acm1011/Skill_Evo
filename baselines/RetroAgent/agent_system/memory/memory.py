...
try:
    import torch
except ImportError:  # pragma: no cover - lightweight environments may not have torch
    torch = None
...
class ReflectionMemory:
    def __init__(self, 
        ...
        self.use_embeddings = SentenceTransformer is not None
        ...
        if self.use_embeddings:
            ...
            print("sentence-transformers not available, using lexical fallback for ReflectionMemory.")
            self.model = None
    ...
    def _compute_similarity_single(self, text_a: str, text_b: str) -> float:
        ...
        set_a = set(str(text_a or "").lower().split())
        set_b = set(str(text_b or "").lower().split())
    ...
    def add(self, 
        ...
        if self.use_embeddings:
            ...
            if self.task_embeddings is None:
                self.task_embeddings = [task_description]
            else:
                self.task_embeddings.append(task_description)
    ...
    def retrieve(self, current_task_description: str, top_k=3, filter_type="both") -> List[Dict[str, str]]:
        ...
        if self.use_embeddings and self.task_embeddings.shape[0] != len(self.data):
            self._rebuild_embeddings()
        elif not self.use_embeddings and len(self.task_embeddings) != len(self.data):
            self._rebuild_embeddings()
        ...
        for idx, item in enumerate(self.data):
            ...
            relevance = cos_scores[idx].item() if self.use_embeddings else self._compute_similarity_single(current_task_description, item['task_desc'])
            if relevance < 0.4: 
                continue
