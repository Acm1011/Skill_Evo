...
def run_evolve_memory(args: argparse.Namespace) -> int:
    ...
    for traj in trajectories:
        ...
        try:
            ...
            new_records.append(make_memory_record(traj, parsed, raw_teacher_output=raw))
    ...
    if args.output_case_pool:
        write_jsonl(args.output_case_pool, [export_case_pool_row(x) for x in merged_rows])
    if args.output_dummy_memory:
        write_jsonl(args.output_dummy_memory, [export_dummy_memory_row(x) for x in merged_rows])
...
def build_evolve_memory_parser(sub: Any) -> None:
    p = sub.add_parser("evolve-memory", help="Incrementally update MementoMath memory bank")
    ...
    p.add_argument("--output-case-pool", default="")
    p.add_argument("--output-dummy-memory", default="")
