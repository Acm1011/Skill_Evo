...
usage() {
    cat <<USAGE
Usage:
  bash baselines/ExpeLMath/scripts/run_build_memory_with_rollout.sh [options]
...
OUT_PATH="${REPO_ROOT}/baselines/ExpeLMath/outputs/memory_bank.jsonl"
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        -h|--help) usage; exit 0 ;;
        *) echo "[expel-build-memory] unknown arg: $1" >&2; usage; exit 2 ;;
    esac
done

if [[ -z "${TRAJ_PATH}" ]]; then
    echo "[expel-build-memory] --trajectories is required" >&2
    exit 2
fi
...
for ((i = 0; i < SE_ROLLOUT_N_SERVERS; i++)); do
    ...
    if [[ "${ok}" -ne 1 ]]; then
        echo "[expel-build-memory] rollout health check failed: ${url}" >&2
        exit 1
    fi
done
