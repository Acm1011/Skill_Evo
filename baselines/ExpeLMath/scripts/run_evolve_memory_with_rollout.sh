...
usage() {
    cat <<USAGE
Usage:
  bash baselines/ExpeLMath/scripts/run_evolve_memory_with_rollout.sh [options]
...
OUT_MEMORY_BANK="${REPO_ROOT}/baselines/ExpeLMath/outputs/memory_bank_v2.jsonl"
EXISTING_EMBEDDINGS=""
OUT_EMBEDDINGS=""
EMBED_BACKEND="hash"
EMBED_BASE_URL=""
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        -h|--help) usage; exit 0 ;;
        *) echo "[expel-evolve-memory] unknown arg: $1" >&2; usage; exit 2 ;;
    esac
done

if [[ -z "${MEMORY_BANK}" || -z "${TRAJ_PATH}" ]]; then
    echo "[expel-evolve-memory] --memory-bank and --trajectories are required" >&2
    exit 2
fi
...
for ((i = 0; i < SE_ROLLOUT_N_SERVERS; i++)); do
    ...
    if [[ "${ok}" -ne 1 ]]; then
        echo "[expel-evolve-memory] rollout health check failed: ${url}" >&2
        exit 1
    fi
done
