...
usage() {
    cat <<USAGE
Usage:
  bash baselines/ExpeLMath/scripts/run_gen_traj.sh [options]
...
OUTPUT="${REPO_ROOT}/baselines/ExpeLMath/outputs/trajectories.jsonl"
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        -h|--help) usage; exit 0 ;;
        *) echo "[expel-gen-traj] unknown arg: $1" >&2; usage; exit 2 ;;
    esac
done

if [[ -z "${DATA_PATH}" || -z "${END_IDX}" ]]; then
    echo "[expel-gen-traj] --data-path and --end are required" >&2
    exit 2
fi
...
for ((i = 0; i < SE_ROLLOUT_N_SERVERS; i++)); do
    ...
    if [[ "${ok}" -ne 1 ]]; then
        echo "[expel-gen-traj] rollout health check failed: ${url}" >&2
        exit 1
    fi
done
