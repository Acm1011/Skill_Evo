...
export MKL_SERVICE_FORCE_INTEL
...
python3 run_ppo_math.py \
  ...
  rewards.weights.format="${RW_FORMAT}" \
  rewards.weights.outcome=1.0 \
  rewards.weights.info_gain=0 \
  rewards.weights.experience="${RW_EXPERIENCE}" \
  ...
  experience.embedding_api_key="${EMBEDDING_API_KEY}" \
  experience.embedding_model="${EMBEDDING_MODEL}" \
