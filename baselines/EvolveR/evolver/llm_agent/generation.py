...
def _knowledge_search_enabled() -> bool:
    """Default off for baselines/EvolveR (no Wiki). Set EVOLVER_KNOWLEDGE_SEARCH=1 to call retriever HTTP."""
    return os.environ.get("EVOLVER_KNOWLEDGE_SEARCH", "0") == "1"
...
class LLMGenerationManager:
    ...
    def execute_predictions(self, predictions: List[str], pad_token: str, active_mask=None, do_search=True) -> List[str]:
        ...
        if do_search and knowledge_queries and _knowledge_search_enabled():
            knowledge_results, original_knowledge_results = self.batch_search(knowledge_queries)
            assert len(knowledge_results) == sum(
                [1 for action in cur_actions if action == "search_knowledge"]
            )
        else:
            nk = sum([1 for action in cur_actions if action == "search_knowledge"])
            knowledge_results = [""] * nk
            original_knowledge_results = [{}] * nk
            ...
            if not active:
                ...
                if action == 'answer':
                    ...
                    if self.config.experience_enabled:
                        if _knowledge_search_enabled():
                            next_obs.append(
                                "\nMy previous action is invalid. "
                                "To search knowledge use <search_knowledge>query</search_knowledge>. "
                                "To search experience use <search_experience>query</search_experience>. "
                                "For the final answer use <answer>...</answer>. Let me try again.\n"
                            )
                        else:
                            next_obs.append(
                                "\nMy previous action is invalid. "
                                "To retrieve past math-solving principles use <search_experience>query</search_experience>. "
                                "For the final answer use <answer>...</answer>. Let me try again.\n"
                            )
                    else:
                        if _knowledge_search_enabled():
                            ...
                            next_obs.append(
                                "\nMy previous action is invalid. "
                                "Put reasoning in <think>...</think> and the final result in <answer>...</answer>. Let me try again.\n"
                            )
                    dones.append(0)
                    valid_action.append(0)
                    is_search.append(0)
