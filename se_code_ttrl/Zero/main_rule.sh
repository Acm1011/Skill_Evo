...
variant=prompt2_C20_rule_se-Zero_${base_model_name}
exp_name=${variant}-V1
...
echo "开始第一轮训练..."
echo "训练 Challenger..."
# bash ${SCRIPT_DIR}/challenger_rule.sh $exp_name $base_model_path $base_model_path $challenger_training_steps $topics_path || {
...
for iter in {2..6}; do
    ...
    echo "开始第 ${iter} 轮训练..."
    echo "训练 challenger_rule (${exp_name})..."
    bash ${SCRIPT_DIR}/challenger_rule.sh \
