...
# 解析参数
exp_name=test_challenger_entropy_base
challenger_model_path=/root/users/ycy/models/shares/Qwen3-4B-Base
solver_model_path=/root/users/ycy/models/shares/Qwen3-4B-Base
challenger_training_steps=5
topics_path=/root/users/ycy/saved_results/prompt2_se-Zero_Qwen3-4B-Base/tmp/topics.json
...
bash ${SCRIPT_DIR}/challenger_entropy_reward.sh $solver_model_path 
# 等待vLLM服务启动并跟踪进程
echo "等待vLLM服务启动..."
...
CUDA_VISIBLE_DEVICES=0,1,2,3 python3 -m se_code.main_challenger \
    ...
    reward_model.reward_manager=challenger_entropy \
