...
class DatasetHandler(ABC):
    ...
    def get_score(self, responses: str, answers: str) -> float:
        scores = []
        preds = [self.extract_answer(response[-200:]) for response in responses]
        for r,a in zip(responses, answers):
            if self.compare_answer(r,a):
                scores.append(1)
            else:
                scores.append(0)
        return scores, preds, sum(scores)/len(scores)
...
class MathDatasetHandler(DatasetHandler):
    ...
    def load_data(self):
        ...
        # )
        data_path=os.path.join(data_dir, 'MATH-500')
...
class Gsm8kDatasetHandler(DatasetHandler):
    ...
    def load_data(self):
        ...
        dataset = load_dataset(data_path, 'main', split='test')
        examples = [row for row in dataset]
...
class AmcDatasetHandler(DatasetHandler):
    ...
    def load_data(self):
        data_path=os.path.join(data_dir, 'amc23')
...
class MinervaDatasetHandler(DatasetHandler):
    ...
    def load_data(self):
        data_path=os.path.join(data_dir, 'minerva')
...
class OlympiadDatasetHandler(DatasetHandler):
    ...
    def load_data(self):
        data_path=os.path.join(data_dir, 'olympiadbench')
...
class Aime2024DatasetHandler(DatasetHandler):
    ...
    def load_data(self):
        data_path=os.path.join(data_dir, 'AIME_2024')
...
class Aime2025DatasetHandler(DatasetHandler):
    ...
    def load_data(self):
        data_path=os.path.join(data_dir, 'aime_2025')
        #dataset = load_dataset("yentinglin/aime_2025", 'default')['train']
        dataset = load_dataset(data_path, 'default', split='train')
        examples = [row for row in dataset]
        questions = [example['problem'] for example in examples]*32
        answers = [example['answer'] for example in examples]*32
