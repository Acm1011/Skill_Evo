...
@retry(stop=stop_after_attempt(3), wait=wait_fixed(5))  # 最多尝试3次，每次重试间隔5秒
...
def process_data_item(args):
    ...
    # (i,'amc', data_item['question'], data_item['response'], data_item['answer'],data_item['score'])
    i,data_source, problem,responses, gts,scores = args
    # 确保gts是Python原生类型
...
def post_eval(save_path_dir, dataset_name, model_name, n_samples):
    #dataset = pd.read_parquet(save_path)
    path='results_minerva.json'
    dataset_name=path.split('_')[-1].split('.')[0]
    with open(path, 'r') as f:
        dataset = json.load(f)[:-1]
    ...
    with ThreadPoolExecutor(max_workers=min(os.cpu_count(),100)) as executor:
        args = [
            (i,dataset_name, data_item['question'], data_item['response'], data_item['answer'],data_item['score'])
            for i, data_item in enumerate[Any](dataset)
        ]
        ...
        for data_source, data in results.groupby('data_source'):
            ...
            output_path = f'results_{dataset_name}_final.json'
            ...
            raw_score = sum([example['raw_scores'][0] for example in data]) / data_source_len if data_source_len > 0 else 0
            mean_rule_score = sum([
    ...
    overall_results_path = os.path.join(f'oly_Overall_results.jsonl')
...
if __name__ == "__main__":
    ...
    post_eval(args.save_path_dir, args.dataset, args.model_name, args.n_samples)