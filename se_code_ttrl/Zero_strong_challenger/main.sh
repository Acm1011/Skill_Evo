...
# base_model_name="${SE_BASE_MODEL_NAME:-Qwen3-4B-Base}"
...
# R_Zero_ref_q
question_reward=R_Zero_ref_q
strong_challenger_name=Qwen2.5-14B-Instruct
strong_challenger=${model_dir}/${strong_challenger_name}
...
echo "[配置] dataset: ${dataset}, solver_batch_size: ${solver_batch_size}, rollout_n: ${rollout_n}, train_file: ${train_file}"
...
variant=sc_${strong_challenger_name}_data_${dataset}_${question_reward}_gq${gen_question_func}_${base_model_name}
...
# echo "训练 Solver..."
# bash ${SCRIPT_DIR}/solver.sh $exp_name ${strong_challenger} $base_model_path $solver_training_steps $gen_question_func $hybrid_data  $train_file $solver_batch_size $real_data_ratio $rollout_n || {
