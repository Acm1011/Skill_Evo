...
base_model_name="${SE_BASE_MODEL_NAME:-Llama-3.2-3B-Instruct}"
...
solver_retrain_steps=10
solver_eval_step=10
solver_eval_temperature=0.6
solver_eval_num_iter=20
...
# echo "训练 Solver..."
# bash ${SCRIPT_DIR}/solver.sh $exp_name ${challenger_path_dir}/${exp_name}/ckpts/global_step_${challenger_training_steps}/actor/huggingface $base_model_path $solver_training_steps $gen_question_func $hybrid_data  $train_file $solver_batch_size $real_data_ratio|| {
#     echo " Error: 第一轮 Solver 训练失败"
#     exit 1
# }


for iter in $(seq 13 ${solver_eval_num_iter}); do
    prev=$((iter-1))
    ...
    bash ${SCRIPT_DIR}/solver.sh \
        ...
        ${prev_solver_model_path} $solver_training_steps $gen_question_func $hybrid_data $train_file $solver_batch_size $real_data_ratio || {
