...
def data_merge(args):
    ...
    for item in real_data:
        ...
        if problem not in seen_problems:
            ...
            sample_real_data.append(item)
    
    real_data_cnt = round(len(sample_real_data) * args.real_data_ratio)
    if args.real_data_ratio <= 1.0:
        sampled_real_data = random.sample(sample_real_data, k=real_data_cnt) # 无放回采样
    else:
        sampled_real_data = random.choices(sample_real_data, k=real_data_cnt) # 有放回
    print(f' real data num: {len(sample_real_data)}, raw real data: {len(real_data)}, real data sample ratio: {args.real_data_ratio}, sampled real data: {len(sampled_real_data)}')
    ...
    print(f'采样后的真实数据: {len(sampled_real_data)}, 合成数据去重前: {synthetic_cnt_before}, 去重后: {len(unique_synthetic_data)}')
    
    # 保证真实数据出现在前 2 * len(real_data) 的范围内
    # 取 1 * len(real_data) 个合成数据与真实数据混合
    front_synthetic_cnt = min(len(sampled_real_data), len(unique_synthetic_data))
    ...
    front_data = sampled_real_data + front_synthetic
    print(f'前半部分数据: {len(front_data)}, 前半部分真实数据: {len(sampled_real_data)}, 前半部分合成数据: {len(front_synthetic)}, 后半部分合成数据: {len(remain_synthetic)}')
...
if __name__ == "__main__":
    ...
    print(f"real_data_ratio: {args.real_data_ratio}")
